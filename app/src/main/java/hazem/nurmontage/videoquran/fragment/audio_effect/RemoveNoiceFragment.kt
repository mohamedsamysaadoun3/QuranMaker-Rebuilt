...
/**
 * Fragment for toggling noise removal on an audio entity.
 * Builds FFmpeg filter chain with atrim, afftdn, volume, fade, enhance, reverb, echo, speed.
 * Converted from RemoveNoiceFragment.java (199 lines).
 */
class RemoveNoiceFragment() : Fragment() {

    companion object {
        var instance: RemoveNoiceFragment? = null
        ...
        fun getInstance(
            ...
            if (instance == null) {
                instance = RemoveNoiceFragment(callback, entityAudio)
            }
            return instance!!
        }
    }
    ...
    private fun apply(applyAll: Boolean) {
        ...
        effectAudio.isRemoveNoice = btnRemoveNoice?.isChecked ?: false
        ...
        val callback = iEditMediaCallback ?: return
        if (applyAll) {
            callback.updateEntity(EffectAudioType.NOICE, entityAudio!!)

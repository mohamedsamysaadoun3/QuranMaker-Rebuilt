...
/**
 * Adapter for selecting transition effects on Quran ayah entities.
 * Similar to TransitionBismilahAdabters but adds subscription gating:
 * items beyond position 10 show a lock overlay for non-subscribers.
 * Converted from original Java TransitionEntityAdabters (144 lines).
 */
class TransitionEntityAdabters(
    private val isSubscribe: Boolean,
    private val iTransition: EffectAyaFragment.ITransition?,
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        init {
            view.setOnClickListener {
                ...
                if (!isSubscribe && adapterPosition > 10) {
                    iTransition.toSubscribe()
                    return@setOnClickListener
                }
                if (select == adapterPosition) return@setOnClickListener
                val oldPos = select
